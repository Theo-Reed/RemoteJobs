 ...
 * - "面议"
 * 
 * @param {string} salaryStr - 薪资字符串
 * @returns {Object|null} - { min: number, max: number } 或 null（如果无法解析）
 */
...
/**
 * 判断薪资是否匹配筛选条件
 * @param {string} salaryStr - 数据库中的薪资字符串
 * @param {string} filter - 筛选条件：'全部', '10k以下', '10-20K', '20-50K', '50K以上', '项目制/兼职'
 * @returns {boolean} - 是否匹配
 */
function matchSalary(salaryStr, filter) {
  if (filter === '全部') {
    return true
  }
